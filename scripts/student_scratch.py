#!/usr/bin/env python
"""
Train network_s (student architecture) from random initialisation on labelled
neural data, with no distilled or foundation weights.

All parameters (core + readout) are trained jointly with Poisson loss.

Launch with:
    torchrun --nproc_per_node=<num_gpus> scripts/student_scratch.py [config]
"""
...
DEFAULT_CONFIG = Path('/project/rf/code/fnn/data/train_digital_twin/config_student_scratch.yaml')
...
def main(args):
    ...
    # BUILD MODEL — random init, all parameters trainable
    if is_main:
        logger.info("Building network_s from random initialisation.")
    torch.cuda.empty_cache()
    model = network_s(units=units).to(device)

    # Broadcast random init from rank 0 so all ranks start identically
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train network_s from random init on labelled neural data."
    )
    parser.add_argument(
        "config",
        type=Path,
