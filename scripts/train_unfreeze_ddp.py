#!/usr/bin/env python
"""
Train a digital twin with ALL parameters unfrozen (core + modulation.lstm trainable).

Compared to train_digital_twin_ddp.py, the transferred foundation weights are NOT
frozen, so the full network is fine-tuned end-to-end.  Evaluation (CC_abs, CC_max,
CC_norm) runs automatically on rank 0 after training completes.

Launch with:
    torchrun --nproc_per_node=<num_gpus> scripts/train_unfreeze_ddp.py [config]
"""
...
def main(args):
    ...
    # TRANSFER CORE — weights copied but NOT frozen (full fine-tuning)
    if is_main:
        logger.info("Transferring foundation core (unfrozen).")
    ...
    if is_main:
        ...
        # Evaluate best checkpoint (preferred) then final
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fine-tune all parameters (unfrozen) with multi-GPU DDP + post-training eval."
    )
    parser.add_argument(
        "config",
        type=Path,
